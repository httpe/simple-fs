...
typedef struct block_storage {
    ...
    int64_t (*read_blocks)(struct block_storage* storage, uint8_t* buff, uint32_t LBA, uint32_t block_count); // return bytes read, 0 means error
    int64_t (*write_blocks)(struct block_storage* storage, uint32_t LBA, uint32_t block_count, const uint8_t* buff); // return bytes written,  0 means error
